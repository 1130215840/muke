...
export class CourseService {
  ...
  protected convertDateFromClient(course: ICourse): ICourse {
    const copy: ICourse = Object.assign({}, course, {
      courseStartTime: course.courseStartTime != null && course.courseStartTime.isValid() ? course.courseStartTime.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.courseStartTime = res.body.courseStartTime != null ? moment(res.body.courseStartTime) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((course: ICourse) => {
        course.courseStartTime = course.courseStartTime != null ? moment(course.courseStartTime) : null;
      });
    }
    return res;
  }
