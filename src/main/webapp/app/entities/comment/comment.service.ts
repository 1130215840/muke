...
export class CommentService {
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    const copy: IComment = Object.assign({}, comment, {
      commentTime: comment.commentTime != null && comment.commentTime.isValid() ? comment.commentTime.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.commentTime = res.body.commentTime != null ? moment(res.body.commentTime) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.commentTime = comment.commentTime != null ? moment(comment.commentTime) : null;
      });
    }
    return res;
  }
